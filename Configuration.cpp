...
void CConfig::Save()
{
	...
	if (SUCCEEDED(SHGetFolderPath(NULL, CSIDL_MYDOCUMENTS, NULL, 0, path)))
	{
		...
		folder = std::string(path) + ("\\getrektcracked\\");
		switch (g_Options.Menu.ConfigFile)
		{
		case 0:
			file = std::string(path) + ("\\getrektcracked\\") + std::string("Legit") + XorStr(".xyz");
			break;
		case 1:
			file = std::string(path) + ("\\getrektcracked\\") + std::string("Rage") + XorStr(".xyz");
			break;
		case 2:
			file = std::string(path) + ("\\getrektcracked\\") + std::string("HvH") + XorStr(".xyz");
			break;
		case 3:
			file = std::string(path) + ("\\getrektcracked\\") + std::string("Other") + XorStr(".xyz");
			break;
		}
